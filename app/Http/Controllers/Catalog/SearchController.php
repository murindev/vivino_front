...
//            })
        ...
        if(array_key_exists('q',\request()->all())){
            $product->where('products.name','like', '%'.\request()->get('q').'%');
...
//            dump($product->toSql());
        ...
        $catalog = $product->paginate('12', ['*'], '', request()->get('page')
        ...
        if(request()->get('type') == 'short'){
            return view('components.searchLink',[
                'catalog' => $catalog->items(),
                'total' => number_format ( $catalog->total() ,  0 , "" ," " )
            ]);
        }
